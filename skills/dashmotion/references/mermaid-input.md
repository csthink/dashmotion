...
- **Recomputed**: all geometry (always top-down), sizes, every color. Mermaid styling (`classDef`, `class`, `:::`, `style`, `linkStyle`, themes, icons) is **dropped** — dashmotion's palette takes over. If the source used styling, mention the drop once **in your chat reply, never inside the artifact** — the delivered HTML must not contain mermaid directive names or syntax. `click` handlers and `%%` comments are dropped silently.
