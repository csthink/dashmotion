...
[English](https://github.com/csthink/dashmotion/blob/main/README.md) | 简体中文
...
一个把纯文字或 Mermaid 变成动画技术图的 Claude skill —— 输出是单个自包含的 HTML/SVG 文件。

把一段纯文字描述——或一份 Mermaid 源码——变成一张动画图:一个 HTML/SVG 文件、零依赖,虚线沿执行方向流动、光点像在途请求一样穿过系统。也就是现代基础设施产品官网(Diagrid、Temporal、Inngest)上那种动效。

[![Dashmotion 动画演示:一个 checkout 请求穿过 Kubernetes 架构——ingress → cart → payments → Postgres,以及 payments 经 Kafka 到 worker 的异步事件](https://github.com/csthink/dashmotion/raw/main/examples/images/k8s-hero.gif)](https://csthink.github.io/dashmotion/)

**[▶ 在线看真实示例 →](https://csthink.github.io/dashmotion/)** —— 未经编辑的真实产物,不是视频。

> ⭐ 如果 dashmotion 帮你省下了手画图的功夫,[点个 star](https://github.com/csthink/dashmotion) 能让更多人发现它。

## 两种模式

| Flow 模式 | Architecture 模式 |
| --- | --- |
| [![flow demo](https://github.com/csthink/dashmotion/raw/main/examples/images/flow-demo.zh.gif)](https://github.com/csthink/dashmotion/blob/main/examples/images/flow-demo.zh.gif) | [![architecture demo](https://github.com/csthink/dashmotion/raw/main/examples/images/architecture-demo.zh.gif)](https://github.com/csthink/dashmotion/blob/main/examples/images/architecture-demo.zh.gif) |

## 它能做什么

项目名即实现:`stroke-dashoffset` offset 动画 + `animateMotion` 路径,零依赖库、不渲染 GIF、不需要设计工具。
...
**claude.ai** — 从 [Releases](https://github.com/csthink/dashmotion/releases) 下载 `dashmotion.zip`,然后 **Settings → Capabilities → Skills → + Add → 上传 → 开启**。
...
**查看当前装的是哪个版本** —— 版本号在 skill 的 `SKILL.md` 里,跟最新 [release](https://github.com/csthink/dashmotion/releases) 比一下就知道是否最新(`npx skills list` 只显示路径、不显示版本):

```bash
grep '^version:' ~/.claude/skills/dashmotion/SKILL.md     # 项目级:./.claude/skills/dashmotion/SKILL.md
```

**彻底省心:每次启动自动更新(Claude Code)** —— 挂一个 `SessionStart` hook,让 Claude Code 每次启动时刷新这个 skill。写进 `~/.claude/settings.json`:
...
![一段 Mermaid 流程图转成的 dashmotion 动画图](https://github.com/csthink/dashmotion/raw/main/examples/images/mermaid-demo.zh.gif)
...
动画技术、确定性布局引擎、仓库结构,以及导出 GIF/MP4,都在 **[docs/how-it-works.zh-CN.md](https://github.com/csthink/dashmotion/blob/main/docs/how-it-works.zh-CN.md)**。版本历史见 **[CHANGELOG.md](https://github.com/csthink/dashmotion/blob/main/CHANGELOG.md)**。
